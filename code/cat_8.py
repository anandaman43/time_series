"""
This code checks if different moving averages (order 6, 9, 12, 52) and arima (order=(0, 1, 1)) works better than moving
average of order 12 in category VIII on the basis of RMSE and plotting the predictions
"""
import pandas as pd
...
def moving_average(input_df, order=12):
    """
    This function calculates prediction values of last 16 points ( ~ 4 months of test data) using moving average of
    given order or less (if available points before test data is less than order)
    :param input_df: pandas dataframe with two columns - dt_week, quantity : customer-material level data
    :param order: integer
    :return: pandas dataframe with 3 columns - dt_week, quantity and prediction :
    """
...
def moving_average_with_seasonality(input_df, product_seasonal_comp_7_point, order=36):
    ...
    :param product_seasonal_comp_7_point: product level seasonal smoothened data
...
for index, row in sample.iterrows():
    ...
    error_06 = pow(mean_squared_error(df_series["quantity"].iloc[-16:],
                                      result_06["prediction"].iloc[-16:]), 0.5)
    error_09 = pow(mean_squared_error(df_series["quantity"].iloc[-16:],
                                      result_09["prediction"].iloc[-16:]), 0.5)
    error_12 = pow(mean_squared_error(df_series["quantity"].iloc[-16:],
                                      result_12["prediction"].iloc[-16:]), 0.5)
    # error_52 = pow(mean_squared_error(df_series["quantity"].iloc[-16:],
    #                                   result_52["prediction"].iloc[-16:]), 0.5)
    # error_011 = pow(mean_squared_error(df_series["quantity"].iloc[-16:],
    #                                    result_011["prediction"].iloc[-16:]), 0.5)
    error_52 = 0
    error_011 = 0
    report = report.append([[kunag, matnr, error_06, error_09, error_12, error_52, error_011]])
    ...
    plt.plot(result_06.set_index("dt_week").iloc[-16:]["prediction"], marker=".", markerfacecolor="red",
             label="order_06_" + str(error_06))
    plt.plot(result_09.set_index("dt_week").iloc[-16:]["prediction"], marker=".", markerfacecolor="red",
             label="order_09_" + str(error_09))
    plt.plot(result_12.set_index("dt_week").iloc[-16:]["prediction"], marker=".", markerfacecolor="red",
             label="order_12_" + str(error_12))
    # plt.plot(result_52.set_index("dt_week").iloc[-16:]["prediction"], marker=".", markerfacecolor="red",
    #          label="order_52_" + str(error_52))
    # plt.plot(result_011.set_index("dt_week").iloc[-16:]["prediction"], marker=".", markerfacecolor="red",
    #          label="arima_011_" + str(error_011))
    plt.xlabel("Date", fontsize=14)
    plt.ylabel("Quantity", fontsize=14)
    plt.legend()
    # plt.show()
    plt.savefig(folder_address + "/" + str(kunag) + "_" + str(matnr)+".png")
    end = time.time()
    print("time consumed :", (end - start) / 60, " minutes")
report.columns = ["kunag", "matnr", "order_06", "order_09", "order_12", "order_52", "arima_011"]
report.to_csv(file_address, index=False)
