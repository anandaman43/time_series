...
def get_missing_data_weekly(df_grpby_year, grp_min_date, grp_max_date):
    ...
    :param grp_min_date: min_date of data grouped by matnr and kunag
    :param grp_max_date: max_date of data grouped by matnr and kunag
    :return: Return missing data for (kunag, matnr) combo for all years
    """
    ...
    for year, group in df_grpby_year:
        ...
        d = pd.DataFrame(np.zeros((len(_weeks_missed), 2)), columns=['quantity', 'price'])
        missing_data_df_per_grp = pd.concat([d, _weeks_missed_Series], axis=1)
        missing_data_df_per_grp.columns = ['quantity', 'price', 'week_num']
...
def get_missing_data_monthly(df_grpby_year, grp_min_date, grp_max_date):
    ...
    for year, group in df_grpby_year:
        ...
        d = pd.DataFrame(np.zeros((len(_months_missed), 2)), columns=['quantity', 'price'])
        missing_data_df_per_grp = pd.concat([d, _months_missed_Series], axis=1)
        missing_data_df_per_grp.columns = ['quantity', 'price', 'month_num']
...
def get_cmplt_missing_data_weekly(raw_data):
    """
    Get complete missing (weekly aggregated) sets of data for all (kunag, matnr) combo
    ...
    """
    raw_data_grp = raw_data.groupby(['kunag', 'matnr'], as_index=False)
...
def get_cmplt_missing_data_monthly(raw_data):
    """
    Get complete missing (monthly aggregated) sets of data for all (kunag, matnr) combo
    :param raw_data: raw_data as a Pandas dataframe
    :return: Complete missing datasets
    """
    raw_data_grp = raw_data.groupby(['kunag', 'matnr'], as_index=False)
...
def transform_raw_data_weekly(raw_data):
    ...
    raw_data_copy['kunag'] = raw_data_copy['kunag'].map(str)
...
def weekly_aggregate(data):
    ...
    data_grp = data.groupby(['kunag', 'matnr', 'year_weekNum'], as_index=False)[
        ['quantity', 'price']].sum()
...
def monthly_aggregate(data):
    ...
    data = data.drop(['isocalendar'], axis=1)
    data_grp = data.groupby(['kunag', 'matnr', 'year_monthNum'], as_index=False)[
        ['quantity', 'price']].sum()
...
def get_monthly_aggregate_per_product(data):
    # data transform
    data['ds'] = data['ds'].map(str)
    data['date_parse'] = pd.to_datetime(data['ds'])
    data['isocalendar'] = data['date_parse'].map(lambda x: x.isocalendar())
    data = data.drop(['ds', 'date_parse'], axis=1)

    # data aggregation
    data['year_monthNum'] = data['isocalendar'].map(lambda x: (x[0], make_date(','.join(map(str, x))).month))
    data = data.drop(['isocalendar'], axis=1)
    data_grp = data.groupby(['year_monthNum'], as_index=False)[
        ['y']].sum()
    data_grp['ds'] = data_grp['year_monthNum'].map(lambda x: date(int(x[0]), int(x[1]), 15))
...
if __name__ == "__main__":
    # import pandas as pd
    #
    # d = {'kunag': ["A", "A"], 'matnr': ["F", "F"], 'date': ["20180707", "20180707"], 'quantity': [3, 4],
    #      'price': [3, 4]}
    # df = pd.DataFrame(d)
    # print(df)
    # print(get_monthly_aggregate(df))
    from framework import remove_negative_rows
    from framework import individual_series

    # d = {'kunag': ["A", "A", "A"], 'matnr': ["F", "F", "F"], 'date': ["20180707", "20180828", "20180921"], 'quantity': [-3, 4, 5],
    #      'price': [3, 4, 5]}
    # df = pd.DataFrame(d)
    # print(df)
    # print(get_weekly_aggregate(df))
    import pandas as pd
