    ...
    for parameters in tqdm(itertools.product(p, d, q)):
        try:
            ...
            for index, row in bucket_1_sample.iterrows():
                ...
                mse += models.prophet(df, row["kunag"], row["matnr"], growth=parameters[0],
                                      changepoint_prior_scale=parameters[1],seasonality_prior_scale=parameters[2])[1]
            end = time.time()
            ...
            print("saved1")
            results.to_csv("/home/aman/Desktop/CSO_drug/file_generated/prophet_results.csv")
            print("saved2")
        except:
            pass